...
export class Helper{
    ...
    private priceCache: Map<string, number> = new Map<string, number>();
    private web3: Web3;
    ...
    async getBalanceBNB(address: string): Promise<number> {
    ...
    async getBalanceORI(pKey: string){
        const wallet = new ethers.Wallet(pKey);
        const account = wallet.connect(new ethers.providers.JsonRpcProvider(config.RPC_NODE));
    ...
    async getPoolPrice(token: string): Promise<any>{
    ...
    async getPoolReserves(pool: string): Promise<any> {
    ...
    async getPoolVolume(token: string, baseToken: string): Promise<number> {
    ...
    async getPrice(token: string):Promise<number> {
        let stats = await fetch(`https://www.binance.com/api/v3/ticker/24hr?symbol=${token}USDT`);
    ...
    async getAccountFromPk(pKey: string){
